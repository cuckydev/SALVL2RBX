...
class AssetManager
{
	...
	public:
		...
		bool UploadMesh(std::string name, std::vector<char> data)
		{
			//Open request to mesh upload service
			std::string object = "/ide/publish/UploadNewMesh?name=" + URLEncode(name) + "?description=" + URLEncode("Generated by SALVL2RBX");
			...
			//Try to send request 5 times
			for (int i = 0; i < 5; i++)
			{
				//Setup headers
				HttpAddRequestHeadersA(mesh_request, "Content-Type: */*\n", -1, HTTP_ADDREQ_FLAG_ADD | HTTP_ADDREQ_FLAG_REPLACE);
				HttpAddRequestHeadersA(mesh_request, "User-Agent: Roblox/WinInet\n", -1, HTTP_ADDREQ_FLAG_ADD | HTTP_ADDREQ_FLAG_REPLACE);
				HttpAddRequestHeadersA(mesh_request, ("Cookie: " + cookie + "\n").c_str(), -1, HTTP_ADDREQ_FLAG_ADD | HTTP_ADDREQ_FLAG_REPLACE);
				...
				{
					std::cout << GetHTTPString(mesh_request, HTTP_QUERY_RAW_HEADERS_CRLF) << std::endl;
				}
			}
	...
	if (upload)
	{
		...
		for (auto &i : meshes)
		{
			for (auto &j : i.second.parts)
			{
				...
				if (asset_manager.UploadMesh(j.second.name, meshbuf))
					return 1;
